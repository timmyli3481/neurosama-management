...
export const Route = createFileRoute("/_dashboard/scouting/event/$code")({
  component: ScoutingEventPage,
});
...
function TeamsTab({
  ...
  const teamsMap = new Map<
    number,
    { teamNumber: number; matchCount: number; wins: number; losses: number }
  >();
  ...
  matches.forEach((match) => {
    ...
    redTeams.forEach((t) => {
      const existing = teamsMap.get(t.teamNumber) ?? {
      ...
      teamsMap.set(t.teamNumber, existing);
    });
...
function ScoutingEventPage() {
  ...
  // Sync action
  const syncEventData = useAction(api.integrations.ftcScoutActions.syncEventData);
  ...
  // Auto-sync when event not found
  useEffect(() => {
  ...
  if (isSyncing) {
    return (
      <div className="flex flex-col items-center justify-center py-24">
        ...
        <h2 className="text-xl font-semibold mb-2">Syncing Event Data</h2>
        <p className="text-muted-foreground">
          Fetching event "{code}" from FTC Scout...
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <p className="text-muted-foreground mb-4 text-center max-w-md">
          {syncError ||
            `The event "${code}" could not be found. Please check the event code and try again.`}
        </p>
        <div className="flex gap-2">
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="matches" className="w-full">
        ...
        <TabsContent value="teams" className="mt-4">
          <TeamsTab matches={matches} onTeamClick={handleTeamClick} />
        </TabsContent>
